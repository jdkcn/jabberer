 ...
 * @version $Id$
 */
public class RobotXmppListener implements MessageListener, ChatManagerListener {
	...
	public RobotXmppListener(XMPPConnection connection, Robot robot) {
		...
		if ("/l".equalsIgnoreCase(command) || "/list".equalsIgnoreCase(command)) {
			...
			if (robot.getAdministratorNames().indexOf(sender) == -1) {
				...
				if (args == null || args.length == 0) {
					...
					try {
						...
						connection.getRoster().createEntry(args[0].trim(), nickname, groups);
						robot.getRosters().clear();
						robot.getRosters().addAll(connection.getRoster().getEntries());
					} catch (XMPPException e) {
						e.printStackTrace();
					}
				}
			...
			if (robot.getAdministratorNames().indexOf(sender) == -1) {
				...
				if (args == null || args.length < 2) {
					...
					RosterEntry rosterEntry = connection.getRoster().getEntry(args[0].trim());
					if (rosterEntry != null) {
						rosterEntry.setName(args[1]);
					}
					connection.getRoster().setSubscriptionMode(SubscriptionMode.reject_all);
					robot.getRosters().clear();
					robot.getRosters().addAll(connection.getRoster().getEntries());
				}
			}
			...
			if (robot.getAdministratorNames().indexOf(sender) == -1) {
				...
				if (args == null || args.length < 1) {
					...
					try {
						RosterEntry rosterEntry = connection.getRoster().getEntry(args[0]);
						if (rosterEntry != null) {
